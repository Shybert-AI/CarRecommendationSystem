...
class MultiHeadAttention(nn.Module):
    ...
    def head_split(self, x):  # get dimensions bs * h * seq_len * d_k
        new_x_shape = x.size()[:-1] + (self.h, self.d_k)   #将多头放入
        return x.view(*new_x_shape).transpose(-2, -3)  #transpose 将商品数和商品的维度放在一起

    def forward(self, q, k, v, mask=None):
        origin_shape = q.size()

        # perform linear operation and split into h heads  #q、k、v 进行线性操作并拆分多头
        if not self.kq_same:
