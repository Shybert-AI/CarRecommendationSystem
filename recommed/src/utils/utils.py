...
def batch_to_gpu(batch: dict, device) -> dict:
    for c in batch:
        if type(batch[c]) is torch.Tensor:
            batch[c] = batch[c].long().to(device)
    return batch
...
def eval_list_columns(df: pd.DataFrame) -> pd.DataFrame:
    for col in df.columns:
        if pd.api.types.is_string_dtype(df[col]):  #判断每列数据是否为字符
            df[col] = df[col].apply(lambda x: eval(str(x)))  # some list-value columns  #将每列数据转化为数字
    return df
...
def format_arg_str(args, exclude_lst: list, max_len=20) -> str:
    ...
    key_max_len = max(map(lambda x: len(str(x)), keys))  #统计训练参数中键最大字符长度
    value_max_len = min(max(map(lambda x: len(str(x)), values)), max_len)  #统计训练参数中值最大字符长度
